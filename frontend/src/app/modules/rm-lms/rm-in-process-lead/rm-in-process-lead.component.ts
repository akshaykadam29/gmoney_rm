...
@Component({
  selector: 'app-rm-in-process-lead',
  templateUrl: './rm-in-process-lead.component.html',
  styleUrls: ['./rm-in-process-lead.component.css']
})
export class RmInProcessLeadComponent implements OnInit {
  date:any;
  search = "";
  myDatePicker : any;
  ...
  setRmAllAssignedLeadsById(){
    this.rmLeadService.getRmAllAssignedLeadsById(this.id,"inProcess").subscribe((res : any) => {
  ...
  searchLeadByInput(input :any,searchMethodInput : any){
    ...
    if(inputText.length >= 3){
      this.rmLeadService.getLeadsByUserInput(inputText,"In-Process",this.id).subscribe((response : any) => {
