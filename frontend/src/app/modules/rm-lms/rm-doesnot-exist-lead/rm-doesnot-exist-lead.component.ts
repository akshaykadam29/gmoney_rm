...
@Component({
  selector: 'app-rm-doesnot-exist-lead',
  templateUrl: './rm-doesnot-exist-lead.component.html',
  styleUrls: ['./rm-doesnot-exist-lead.component.css']
})
export class RmDoesnotExistLeadComponent implements OnInit {
  date:any;
  search = "";
  myDatePicker : any;
  ...
  setRmAllAssignedLeadsById(){
    this.rmLeadService.getRmAllAssignedLeadsById(this.id,"doesntExist").subscribe((res : any) => {
  ...
  searchLeadByInput(input :any,searchMethodInput : any){
    ...
    if(inputText.length >= 3){
      this.rmLeadService.getLeadsByUserInput(inputText,"Not-Exist",this.id).subscribe((response : any) => {
