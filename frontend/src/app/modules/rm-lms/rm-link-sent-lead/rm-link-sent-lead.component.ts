...
@Component({
  selector: 'app-rm-link-sent-lead',
  templateUrl: './rm-link-sent-lead.component.html',
  styleUrls: ['./rm-link-sent-lead.component.css']
})
export class RmLinkSentLeadComponent implements OnInit {
  date:any;
  search = "";
  myDatePicker : any;
  ...
  setRmAllAssignedLeadsById(){
    debugger
    this.rmLeadService.getRmAllAssignedLeadsById(this.id,"linkSent").subscribe((res : any) => {
  ...
  searchLeadByInput(input :any,searchMethodInput : any){
    ...
    if(inputText.length >= 3){
      this.rmLeadService.getLeadsByUserInput(inputText,"Link-Sent",this.id).subscribe((response : any) => {
