...
@Component({
  selector: 'app-rm-onboard-lead',
  templateUrl: './rm-onboard-lead.component.html',
  styleUrls: ['./rm-onboard-lead.component.css']
})
export class RmOnboardLeadComponent implements OnInit {
  date:any;
  search = "";
  myDatePicker : any;
  ...
  setRmAllAssignedLeadsById(){
    this.rmLeadService.getRmAllAssignedLeadsById(this.id,"onboarded").subscribe((res : any) => {
  ...
  searchLeadByInput(input :any,searchMethodInput : any){
    ...
    if(inputText.length >= 3){
      this.rmLeadService.getLeadsByUserInput(inputText,"Onboarded",this.id).subscribe((response : any) => {
