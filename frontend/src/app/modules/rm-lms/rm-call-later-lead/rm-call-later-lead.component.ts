...
@Component({
  selector: 'app-rm-call-later-lead',
  templateUrl: './rm-call-later-lead.component.html',
  styleUrls: ['./rm-call-later-lead.component.css']
})

export class RmCallLaterLeadComponent implements OnInit {
  search = "";
  ...
  setRmAllAssignedLeadsById(){
    this.rmLeadService.getRmAllAssignedLeadsById(this.id,"call").subscribe((res : any) => {
  ...
  searchLeadByInput(input :any,searchMethodInput : any){
    ...
    if(inputText.length >= 3){
      this.rmLeadService.getLeadsByUserInput(inputText,"Call-Later",this.id).subscribe((response : any) => {
