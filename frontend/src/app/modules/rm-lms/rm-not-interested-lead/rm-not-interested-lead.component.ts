...
@Component({
  selector: 'app-rm-not-interested-lead',
  templateUrl: './rm-not-interested-lead.component.html',
  styleUrls: ['./rm-not-interested-lead.component.css']
})
export class RmNotInterestedLeadComponent implements OnInit {
  date:any;
  myDatePicker : any;
  search = "";
  ...
  setRmAllAssignedLeadsById(){
    this.rmLeadService.getRmAllAssignedLeadsById(this.id,"notInterested").subscribe((res : any) => {
  ...
  setSingleLeadData(){
    ...
    if(this.assignedLeadResData.length > 0){
      this.isData = true;
      this.historyList =this.assignedLeadResData[0].history;
  ...
  searchLeadByInput(input :any,searchMethodInput : any){
    ...
    if(inputText.length >= 3){
      this.rmLeadService.getLeadsByUserInput(inputText,"Not-Interested",this.id).subscribe((response : any) => {
