...
@Component({
  selector: 'app-centre-approved',
  templateUrl: './centre-approved.component.html',
  styleUrls: ['./centre-approved.component.css']
})
export class CentreApprovedComponent implements OnInit {
...
directRepSelected(event : any){
   if(!event.target.value == false){
     if(event.target.value != "All" ){
      ...
      this.getAllAssignedHopitalById(event.target.value, 4);
    }
    if(event.target.value == "All" ){
      var reporteesId= [];
      ...
      this.rmservice.getAllDirectReporteeHospitalById(reporteesId,4).subscribe((res: any) =>{
